gene=read.table("gene.list",head=F)
colnames(gene)="gene"
A=list.files(pattern="rpkm.txt")
files=A[1:9]  #read files
...
data=merge(gene,data)
data=data[,2:101]
j=1
while(j<=100){
  No[j,i]=length(data[,j][which(data[,j]>3)]) #count number of active genes, RPKM >1
  j=j+1}
